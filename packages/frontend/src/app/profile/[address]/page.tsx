...
function ProfitLossCell({ profitLoss }: { profitLoss: bigint | null }) {
  if (profitLoss == null) {
    return <span className="text-gray-400 text-sm">—</span>;
  }
  const positive = profitLoss >= 0n;
  return (
    <span
      className={`text-sm font-semibold ${positive ? "text-emerald-600" : "text-rose-600"}`}
    >
      {positive ? "+" : ""}
      {formatAmount(profitLoss)} XLM
    </span>
  );
}

function StakeRow({ stake }: { stake: PortfolioStake }) {
  const callOutcome = stake.call.outcome;
    ...
    <tr className="group border-b border-gray-100 hover:bg-gray-50/70 transition-colors duration-150">
      ...
      <td className="py-3.5 pl-4 pr-3 sm:pl-6">
        <div className="flex flex-col gap-0.5">
          ...
          >
            {stake.call.title || stake.call.description || stake.callId}
          </Link>
      ...
      {/* P&L */}
      <td className="py-3.5 px-3 whitespace-nowrap">
        <ProfitLossCell profitLoss={stake.profitLossStroops} />
      </td>

      {/* Status */}
      <td className="py-3.5 px-3 whitespace-nowrap">
        <StatusTag
          status={stake.status === "ACTIVE" ? "PENDING" : "RESOLVED"}
      ...
      <td className="py-3.5 pl-3 pr-4 sm:pr-6 whitespace-nowrap text-right">
        {stake.transactionHash ? (
          ...
          >
            {stake.transactionHash.slice(0, 6)}…
            {stake.transactionHash.slice(-4)}
          </a>
        ) : (
  ...
  return (
    <div className="py-16 text-center">
      ...
      <p className="text-sm text-gray-500 mt-1">
        Stakes you place on calls will appear here.
      </p>
    </div>
  );
}
  ...
  return (
    <div className="flex items-center justify-between px-4 py-3 sm:px-6 border-t border-gray-100">
      <p className="text-sm text-gray-500">
        ...
        of <span className="font-medium text-gray-700">{total}</span> stakes
      </p>
...
export default function StakesPage() {
  ...
  const [stakes, setStakes] = useState<PortfolioStake[]>([]);
  ...
  const fetchStakes = useCallback(
    async (p: number) => {
      ...
      try {
        ...
        setStakes(portfolio.stakes);
        setTotal(portfolio.total);
      } catch (err) {
        setError(
          err instanceof BackendUnavailableError
            ? "The BACKit API is unavailable — stakes cannot be shown right now."
  ...
  const handleFollowToggle = async (
    ...
    try {
      const res = await fetch(`/api/users/${targetAddress}/${action}`, {
        method: "POST",
      });
      if (res.ok) {
  ...
  const handleFollow = async () => {
    if (!address) return;
    try {
      const res = await fetch(`/api/users/${address}/follow`, {
        method: "POST",
      });
      if (res.ok) {
        fetchProfile();
  ...
  const handleUnfollow = async () => {
    if (!address) return;
    try {
      const res = await fetch(`/api/users/${address}/unfollow`, {
        method: "POST",
      });
      if (res.ok) {
        fetchProfile();
  ...
  const filtered = stakes.filter((s) => {
    const statusMatch =
      statusFilter === "ALL" ||
      (statusFilter === "PENDING" && s.status === "ACTIVE") ||
      (statusFilter === "RESOLVED" && s.status !== "ACTIVE");
                ...
                <table className="min-w-full divide-y divide-gray-100">
                  <thead className="bg-gray-50">
                    <tr>
                      <th
                        scope="col"
                        className="py-3 pl-4 pr-3 sm:pl-6 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider"
                      >
                        Call
                      </th>
                      <th
                      ...
                      <th
                        scope="col"
                        className="py-3 pl-3 pr-4 sm:pr-6 text-right text-xs font-semibold text-gray-500 uppercase tracking-wider"
                      >
                        Tx
                      </th>
                    </tr>
