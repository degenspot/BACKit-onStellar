...
import { getUserProfile } from "@/lib/fixtures/profileFixtures";
...
const generateMockUser = (index: number, baseAddress: string): User => {
  ...
  const address =
    addresses[index % addresses.length] + (index * 1000).toString().slice(-3);
  return {
    address,
...
const mockFollowers = Array.from({ length: 200 }, (_, i) =>
  generateMockUser(i, "follower"),
);
const mockFollowing = Array.from({ length: 150 }, (_, i) =>
  generateMockUser(i, "following"),
);
