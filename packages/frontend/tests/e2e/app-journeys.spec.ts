...
/** Must match NEXT_PUBLIC_BACKEND_URL used by the dev server under test. */
const BACKEND_URL =
...
test.describe("BACKit Critical Journeys", () => {
  test.beforeEach(async ({ page }) => {
    ...
    // Intercept/mock the backend market detail endpoint.
    // Odds are derived from the pool totals, so there is no odds route to mock.
    await page.route(`${BACKEND_URL}/calls/call-1`, async (route) => {
      await route.fulfill({
        status: 200,
        contentType: "application/json",
        body: JSON.stringify({
          ...
          thesis: "Bitcoin will reach $70,000 by June 2026.",
          stakeToken: "USDC",
          totalYesStake: "12000.0000000",
          totalNoStake: "8000.0000000",
          outcome: "PENDING",
          status: "OPEN",
    ...
    // Intercept/mock the backend market activity endpoint
    await page.route(`${BACKEND_URL}/calls/call-1/stakes*`, async (route) => {
      await route.fulfill({
        status: 200,
        contentType: "application/json",
        body: JSON.stringify([]),
    ...
    // Intercept/mock the backend stake ledger and payout ledger
    await page.route(
      `${BACKEND_URL}/users/${TEST_WALLET_ADDRESS}/stakes*`,
      async (route) => {
        await route.fulfill({
          status: 200,
          contentType: "application/json",
    ...
    await page.route(
      `${BACKEND_URL}/users/${TEST_WALLET_ADDRESS}/payouts*`,
      async (route) => {
        await route.fulfill({
          status: 200,
          contentType: "application/json",
  ...
  test("should load landing page correctly and have a clickable CTA", async ({
    page,
  }) => {
    await page.goto("/");
  ...
  test("should connect wallet using mocked window.freighterApi and navigate to profile", async ({
    page,
  }) => {
    ...
    const connectedBtn = page.getByRole("button", {
      name: /Freighter Tester/i,
    });
    ...
    await expect(
      page.getByText("Mock user for testing Freighter wallet connection."),
    ).toBeVisible();
  });
  ...
  test("should load feed page and display market cards", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: /Prediction Feed/i }),
    ).toBeVisible();
    // Check if the mocked call appears
    await expect(page.getByText("BTC > $70k by June 2026")).toBeVisible();
  });

  test("should navigate to market detail page and show correct data", async ({
    page,
  }) => {
    await page.goto("/feed");
    ...
    await expect(
      page.getByText("BTC > $70k by June 2026").first(),
    ).toBeVisible();
    await expect(
      page.getByText("Bitcoin will reach $70,000 by June 2026."),
    ).toBeVisible();
  });

  test("should support back and forth navigation correctly", async ({
    page,
  }) => {
    await page.goto("/");
